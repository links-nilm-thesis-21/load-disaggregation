...
def append_features(rising_features, falling_features, rising_features_aux, falling_features_aux, ris_compatibility_idx, fall_compatibility_idx):
    # Rising features
...
appliances = ['oven1', 'oven2', 'microwave', 'kitchen outlets1', 'kitchen outlets2', 'bathroom gfi', 'washer dryer1', 'washer dryer2', 'fridge']
...
prob_tie_break_ris = pd.DataFrame(index=index)  # probability tie break matrix (point system)
...
num_last_steady_sts = 4  # Number of steady states to save the euclidean distances mean
...
for i in range(len(data[test_index:test_index + window_length])):
    power_samples['P_t'].append(data[appliance][test_index:test_index + window_length].iloc[i])
    if len(power_samples['P_t']) >= window_size:
        df_power_samples = pd.DataFrame(power_samples, index=data[appliance].iloc[test_index:test_index + window_length].index[i - window_size + 1:i + 1])
        ...
        if not {0, 1}.issubset(np.unique(clusters.labels_)):  # To check if there are at least 2 steady states and one transient (-1) (equivalent set([0, 1]).issubset(np.unique(clusters.labels_)))
            ...
            if window_size % 2000 == 0:  # If the window size increases more than 2000 samples, start with a new window to avoid the overload for the DBSCAN calculation
                print(window_size)
            ...
            if END_ACTIVE_CYCLE:  # If active cycle has ended start event matching
                ...
                if len(event_match_aux.columns) == 0:  # Meaning that in that active cycle the events detected where noise (low power events)
                    ...
                    for event in event_match_aux.sum(axis=0).iloc[np.where(event_match_aux.sum(axis=0) > 1)].index:  # The indexing means where the sum of the columns (events) is greater than one. And it means that that event has more than one correspondence
                        ...
                        for app in apps:
                            ...
                            if features['transition'][event] > 0:  # Rising event
                                ...
                                app_prob.append(prob1 + prob2 + prob3 + prob4)
                    ...
                    if match_cond:
                        ...
                        rising_events, falling_events, rising_features, falling_features, rising_features_aux, falling_features_aux = append_features(rising_features, falling_features, rising_features_aux, falling_features_aux, 0, len(falling_events) - 1)
                    while len(rising_events) != 0 and len(falling_events) != 0:  # It means that not all rising/falling events have found a match
                        ...
                        if (condition_1 or condition_2) and condition_3:
                            ...
                            prediction_dict['appliance'].append(power_inter_look_up[fall_appliance_idx])
                            prediction_dict['On_Off'].append(0)
                            rising_events, falling_events, rising_features, falling_features, rising_features_aux, falling_features_aux = append_features(rising_features, falling_features, rising_features_aux, falling_features_aux, ris_compatibility_idx, fall_compatibility_idx)
                            ...
                            for fall_index in adjacent_events[1:]:  # Evaluate the remaining event excluding the first element that was already checked
                                ...
                                if (condition_1 or condition_2) and condition_3:  # A match was found
                                    ...
                                    prediction_dict['appliance'].append(power_inter_look_up[fall_appliance_idx])
                                    prediction_dict['On_Off'].append(0)
                                    rising_events, falling_events, rising_features, falling_features, rising_features_aux, falling_features_aux = append_features(rising_features, falling_features, rising_features_aux, falling_features_aux, ris_compatibility_idx, fall_index)
                                    break
...
# Store the results of appliance predictions in a dataframe
